...
func TestGlobalNetworkFirewallPolicy(t *testing.T) {
	...
	fwp.DefineVerify(
		func(assert *assert.Assertions) {
			...
			assert.Equal(1, len(targetServiceAccounts3), "Rule3 should have the correct service account count")
			...
			assert.Equal("egress-101", sp101.Get("ruleName").String(), "Rule101 ruleName should be ingress-1")
			assert.Equal("test egress rule 101", sp101.Get("description").String(), "Rule101has expected description")
			assert.Equal("EGRESS", sp101.Get("direction").String(), "Rule101 direction should be INGRESS")
			...
			assert.Equal(1, len(destAddressGroups), "rule101 should have the correct destination address group count")
			...
			assert.Equal("egress-102", sp102.Get("ruleName").String(), "Rule102 ruleName should be ingress-1")
			assert.Equal("test egress rule 102", sp102.Get("description").String(), "Rule102 has expected description")
			assert.Equal("EGRESS", sp102.Get("direction").String(), "Rule102 direction should be INGRESS")
			...
			assert.Equal("10.100.0.2/32", sp102.Get("match.destIpRanges").Array()[0].String(), "has expected srcIpRanges")
			...
			assert.Equal("egress-103", sp103.Get("ruleName").String(), "Rule103 ruleName should be ingress-103")
			assert.Equal("test ingres rule 103", sp103.Get("description").String(), "Rule103 has expected description")
			assert.Equal("EGRESS", sp103.Get("direction").String(), "Rule103 direction should be INGRESS")
			...
			assert.Equal("10.100.0.103/32", sp103.Get("match.destIpRanges").Array()[0].String(), "has expected srcIpRanges")
			...
			assert.Equal(3, len(layer4ConfigsPorts103), "Rule3 should have the correct layer4Configs port count")
			targetServiceAccounts103 := sp103.Get("targetServiceAccounts").Array()
			assert.Equal(1, len(targetServiceAccounts103), "Rule3should have the correct service account count")
