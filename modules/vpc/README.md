...
| bgp\_always\_compare\_med | If set to true, the Cloud Router will use MED values from the peer even if the AS paths differ. Default is false. | `bool` | `null` | no |
| bgp\_best\_path\_selection\_mode | Specifies the BGP best path selection mode. Valid values are `STANDARD`, `EARLY_EXIT`, `MULTI_EXIT_DISC`. Default is `STANDARD`. | `string` | `null` | no |
| bgp\_inter\_region\_cost | Specifies the BGP inter-region cost mode. Valid values are `DEFAULT`, `LOW`. Default is `DEFAULT`. | `string` | `null` | no |
