...
locals {
  ...
  sftp_hosts = { for key, value in local.layer_02_outputs.s4pce_customer_instances.raw_instance_list : key => value if lookup(value, "productname", "undefined") == "sftp_interface" }
  ...
  sftp_subnets_map = flatten(concat([
    for key, value in local.sftp_sid_map : [
      for key_network, value_network in local.layer_00_outputs.infrastructure.network : [
        ...
        } if length(regexall("^${replace("${lower(value[0].landscape)}", "-", "_")}", key_subnets)) > 0
      ] if value_network.subnets != null
  ]]))

  sftp_subnets_list = [
    for key, value in local.sftp_subnets_map :
    value.id
  ]

  sftp_endpoints = {
    for key, value in local.sftp_sid_map : lower("${substr(value[0].key, 0, 4)}sftp${key}") => {
      nlb_subnet_list      = local.sftp_subnets_list
      nlb_name             = "${local.base_context.customer}-${lower(key)}-sftp"
      nlb_port_list        = ["8022"]
      nlb_target_id        = [for host in value : local.layer_02_outputs.s4pce_customer_instances.instance_list[host.key].instance_id]
