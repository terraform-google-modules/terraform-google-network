...
locals {
  ...
  enable_ipv6 = var.network.primary.ipv6_ipam == "aws" ? true : false
  ...
  subnet_nonedge_list = flatten(concat([
    for key_network, value_network in var.network : [
      for key_subnets, value_subnets in value_network.subnets : {
        ...
        ipv6_netnum = value_subnets.ipv6_netnum
  }] if value_network.subnets != null]))
  subnet_nonedge_map = { for key, value in local.subnet_nonedge_list : value.subnet_cidr => value }
  subnet_edge_list = flatten(concat([
    for key_network, value_network in var.network : [
      for key_subnets_edge, value_subnets_edge in value_network.subnets_edge : {
        ...
        ipv6_netnum = value_subnets_edge.ipv6_netnum
  }] if value_network.subnets_edge != null]))
  subnet_edge_map = { for key, value in local.subnet_edge_list : value.subnet_cidr => value }
