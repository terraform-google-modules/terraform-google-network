...
variable "windows_versions" {
  description = "A list of Windows versions to approve patches for. A maximum of 20 versions may be defined."
  default     = ["*"]

  validation {
    condition     = length(var.windows_versions) <= 20
    error_message = "The maximum number of 'windows_versions' that may be defined is 20."
  }
}
