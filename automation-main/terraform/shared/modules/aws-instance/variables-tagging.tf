...
variable "enforce_tags" {
  description = "List of instance Tags to continually enforce"
  type        = list(string)
  default     = []
}
...
### Manual Tags
variable "tag_name" {
  description = "AWS Tag Name of the instance. Conventionally a single word category the instance belongs to. A Random hex string will be added to ensure uniqueness"
  type        = string
  default     = null
}
...
variable "tag_scangroup" {
  description = "Group of systems that should be scanned together during scheduled scan windows"
  type        = string
  default     = null
}
variable "tag_securityboundary" {
  description = "Indicates the security boundary"
  type        = string
  default     = null
}
