...
variable "ubuntu_versions" {
  description = "A list of Ubuntu versions to approve patches for. A maximum of 20 versions may be defined."
  default     = ["*"]

  validation {
    condition     = length(var.ubuntu_versions) <= 20
    error_message = "The maximum number of 'ubuntu_versions' that may be defined is 20."
  }
}
